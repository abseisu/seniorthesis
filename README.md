...
This repository contains the complete codebase and Jupyter notebooks used to produce the empirical results in my senior thesis, *An Information-Theoretic Approach to Portfolio Diversification*.  

The primary objective of this repository is **full reproducibility** for researchers with appropriate data access.
...
Important: This project requires CRSP data accessed via WRDS.
CRSP data is proprietary and not included in this repository.
...
NPEET (Required)
Mutual information estimation uses the KSG estimator from NPEET.
...
Path Configuration
Several notebooks contain absolute paths from the original execution environment.
You must update these paths before running anything.
...
Important:
code/src/paths.py defines PROJECT_ROOT relative to the code/ directory.
You must either:
...
weekly_processing.ipynb
MI / NMI kernel construction

weekly_computation.ipynb
Kernel stabilization and GMRP weight computation
...
NPEET notebooks (validation only)
...
Final Note
This repository is designed to be readable, auditable, and fully reproducible.
With valid CRSP access and correct path configuration, all thesis results can be regenerated exactly.
